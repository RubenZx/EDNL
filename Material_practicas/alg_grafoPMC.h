...
vector<tCoste> DijkstraInv(const GrafoP<tCoste>& G,
                           ...
                           vector<typename GrafoP<tCoste>::vertice>& P)
// Calcula los caminos de coste mínimo entre todos los vértices del grafo G 
// y un destino. En el vector D de tamaño G.numVert() devuelve estos costes 
// mínimos y P es un vector de tamaño G.numVert() tal que P[i] es el último 
// vértice del camino de destino a i.
{
   typedef typename GrafoP<tCoste>::vertice vertice;
   vertice v, w;
   const size_t n = G.numVert();
