            ...
            if(i == j){     // Rellenamos los costes de los grafos
                for (v = 0; v < dimGraph; ++v) {  
                    for (w = 0; w < dimGraph; ++w) {
                        bigGraph[v + dimGraph * i][w + dimGraph * i] = vGrafos[i][v][w];
                    }
                }

            }else{
                if(i < j){  // Rellenamos el coste del transbordo en la diag. sup.
                    for (v = i; v < dimBigGraph; ++v) { 
                        for (w = j; v < dimBigGraph; ++v) { 
                            bigGraph[i][j] = vCostes[i][v];
                        }    
                    }
                } // else{      // Rellenamos el coste del transbordo de la diag. inf.
